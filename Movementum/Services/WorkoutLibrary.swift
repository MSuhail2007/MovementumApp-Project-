...
struct WorkoutLibrary {
    ...
    // Prefer to load exercises from the canonical `exercises.json` via ExerciseLibrary.
    // If the JSON is not present or empty, fall back to the embedded hardcoded list.
    private var allExercises: [Exercise] {
        let specs = ExerciseLibrary.loadExercises(from: "exercises")
        if !specs.isEmpty {
            // Use Intermediate as a sensible default when building WorkoutLibrary exercises
            return specs.map { $0.toAppExercise(for: "Intermediate") }
        }

        // Fallback hardcoded list (kept for compatibility)
        return [
            // Upper Body
